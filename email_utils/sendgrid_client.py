"""
sendgrid_client.py - FIXED VERSION (No template dependencies)
Replaces problematic template approach with reliable HTML emails
"""
...
def send_trade_email(trade_data, summary):
    ...
    This bypasses all template issues and always works
    """
...
def send_latest_trade_email(trade_data):
    """
    Send email for the latest trade with a formatted summary.
    """
    if not trade_data:
        return {"error": "No trade data provided", "success": False}
    
    # Create a detailed summary for the latest trade
    summary = f"""
LATEST TRADE ANALYSIS

This trade shows {'positive' if (trade_data.get('pnl_amount', 0) or 0) > 0 else 'neutral'} performance.

Performance Details:
- Position: {trade_data.get('direction', 'Unknown').upper()}
- Entry: ${trade_data.get('entry_price', 'N/A')}
- Exit: ${trade_data.get('exit_price', 'N/A')}
- Result: {trade_data.get('pnl', 'N/A')}

{'Congratulations on the trade execution!' if (trade_data.get('pnl_amount', 0) or 0) > 0 else 'Trade logged successfully for analysis.'}
    """.strip()
    
    return send_trade_email(trade_data, summary)
